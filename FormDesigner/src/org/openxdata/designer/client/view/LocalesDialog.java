...
public class LocalesDialog extends DialogBox {
	...
	private TextBox addNewLocale(String name, String key, int row) {
		...
		table.getCellFormatter().setHorizontalAlignment(row, 3,
		...
		txtName.addKeyDownHandler(new KeyDownHandler() {
			public void onKeyDown(KeyDownEvent event) {
				...
				if (keyCode == KeyCodes.KEY_ENTER
					...
					moveToNextWidget((Widget) event.getSource(), 0,
							keyCode == KeyCodes.KEY_DOWN);
		...
		txtKey.addKeyDownHandler(new KeyDownHandler() {
			public void onKeyDown(KeyDownEvent event) {
				...
				if (keyCode == KeyCodes.KEY_ENTER
						|| keyCode == KeyCodes.KEY_DOWN)
					moveToNextWidget((Widget) event.getSource(), 1,
							keyCode == KeyCodes.KEY_DOWN);
				else if (keyCode == KeyCodes.KEY_UP)
	...
	private void deleteLocale(Widget widget) {
		if (!Window.confirm("Do you really want to delete the "
				+ getLanguageName(widget) + " " + i18n.language() + "?"))
			return;
	...
	private void save() {
		...
		for (int row = 1; row < rowCount; row++) {
			Locale locale = new Locale(
					((TextBox) table.getWidget(row, 1)).getText(),
