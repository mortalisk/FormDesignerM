...
import org.openxdata.forms.client.LeftPanel.Images;
import org.openxdata.forms.client.controller.DragDropListener;
import org.openxdata.forms.client.controller.FormDesignerDragController;
import org.openxdata.forms.client.controller.FormDesignerDropController;
import org.openxdata.forms.client.controller.IWidgetPopupMenuListener;
import org.openxdata.forms.client.controller.WidgetPropertyChangeListener;
import org.openxdata.forms.client.controller.WidgetPropertySetter;
import org.openxdata.forms.client.controller.WidgetSelectionListener;
...
import org.openxdata.forms.client.util.StyleUtil;
...
import org.openxdata.forms.client.widget.DesignGroupWidget;
import org.openxdata.forms.client.widget.DesignWidgetWrapper;
import org.openxdata.forms.client.widget.PaletteWidget;
import org.openxdata.forms.client.widget.RadioButtonWidget;
import org.openxdata.forms.client.widget.TextBoxWidget;
...
public class DesignGroupView extends Composite implements WidgetSelectionListener,IWidgetPopupMenuListener,DragDropListener,WidgetPropertyChangeListener{
	...
	public void moveRubberBand(Event event){
		try
		{
			...
			if(width < 0){
				DOM.setStyleAttribute(rubberBand.getElement(), "left", event.getClientX()-selectedPanel.getAbsoluteLeft()+OpenXdataConstants.UNITS);
				DOM.setStyleAttribute(rubberBand.getElement(), "width", width * -1 + OpenXdataConstants.UNITS);
			}
			else
				DOM.setStyleAttribute(rubberBand.getElement(), "width", (event.getClientX()-selectedPanel.getAbsoluteLeft())-getRubberLeft()+OpenXdataConstants.UNITS);

			if(height < 0){
				DOM.setStyleAttribute(rubberBand.getElement(), "top", event.getClientY()-selectedPanel.getAbsoluteTop()+OpenXdataConstants.UNITS);
				DOM.setStyleAttribute(rubberBand.getElement(), "height", height * -1 + OpenXdataConstants.UNITS);
			}
			else
				DOM.setStyleAttribute(rubberBand.getElement(), "height", (event.getClientY()-selectedPanel.getAbsoluteTop())-getRubberTop()+OpenXdataConstants.UNITS);
		}
		catch(Exception ex){
	...
	private void initEditWidget(){
		...
		//txtEdit.addStyleName("openxdataforms-label-editor");
	}
	...
	/*protected DesignWidgetWrapper addNewSearchServerWidget(String parentBinding, String text, boolean select){
		...
		DOM.setStyleAttribute(repeat.getElement(), "height","70"+OpenXdataConstants.UNITS);
		DOM.setStyleAttribute(repeat.getElement(), "width","285"+OpenXdataConstants.UNITS);
